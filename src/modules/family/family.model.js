...
const FamilySchema = new mongoose.Schema(
    {
        ...
        head_of_family: {
            ...
            // Mongoose model refs are case-sensitive: the model is
            // registered as "Hof" (see hof.model.js), so a ref of "HOF"
            // silently failed to resolve on .populate().
            ref: "Hof",
...
FamilySchema.methods.addMember = function (userId) {
    if (!userId) {
        logger.warn("Attempted to add null/undefined user ID to family");
        return this;
    }
    ...
    try {
        ...
        logger.error("Invalid user ID format when adding family member:", error);
    }
